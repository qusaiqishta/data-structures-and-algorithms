...
for(let i=0;i<arr.length;i++){
  ...
  for (let j = 0; j < arr[i].length; j++) {
    ...
    if(regex.test(arr[i][j])){
      holdingNumbers+=arr[i][j];
    }
    
  }
  standardFormat.push(holdingNumbers);
}
...
const allHappy = (arr) => {
let happyArray=arr.filter(str=>str.includes(":)"));
