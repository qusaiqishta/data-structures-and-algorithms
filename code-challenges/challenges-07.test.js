...
let AddQuestionMark=arr.map(Element=>{
  return Element + '?';
})
...
for(let i=0;i<arr.length;i++){
  raiseInputTo.push(Math.pow(2,arr[i]));
}
return raiseInputTo;
};
...
let raiseInputTo=[];
arr.forEach(element => {
 raiseInputTo.push(Math.pow(2,element));
...
let raiseInputTo=arr.map(element=>{
