...
// try the last sentence which is '2 gallons fresh water'

const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(element => {
    let firstSlice = element.slice(element.indexOf(' ') + 1); // will return gallons fresh water
    let secondSlice = firstSlice.slice(firstSlice.indexOf(' ') + 1); // will return fresh water which is the desired output
