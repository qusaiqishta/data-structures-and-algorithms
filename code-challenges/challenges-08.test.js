...
const typeNum = (arr) => {
let numberArray=arr.filter(Element=>{
...
const filterStringsWithVowels = (arr) => {
  let vowelArray=arr.filter(Element=>{
    let searchFor=/[auoei]/g;
    if(searchFor.test(Element)){
      return Element;
    }
  })
     return vowelArray;
...
let notIncludeArray=arr.filter(Element=>{
  if(!(forbiddenValues.includes(Element))){
    return Element;
  }
})
return notIncludeArray
};
