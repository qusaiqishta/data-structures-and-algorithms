...
let count=0;
let bigArray=input.map(Element=>{
  let subArray=Element.filter((value,index)=>{
    if(value===target){
      count++;
...
let finalArray=input.map(Element=>{
 let subArray= Element.filter((val,idx) => {
    if(val%5==0 && typeof val=='number'){
      return true;
    }
  });
  let subArray2=subArray.map(Element=>{
...
let findShortest = (data) => {
  ...
  let namesArray=data.map(Element=>{
    return Element.name;
  })
  let result=namesArray.filter(Element=>{
    if(firstName.length>=Element.length){
      firstName=Element;
      return firstName;
    }
  })
